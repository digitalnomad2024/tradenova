...
function SignupContent() {
  ...
  const sendOtp = () => {
    ...
    setError("");
    setStep("otp");
    alert(`Demo OTP sent to +91 ${mobile}: ${code}`);
  };
      ...
      <div className="mx-auto grid max-w-6xl gap-10 px-6 py-12 lg:grid-cols-2 lg:py-16">
        ...
        <div>
          ...
          <p className="mt-2 text-sm text-slate-400">
            {step === "mobile" &&
              "We’ll send a one-time password to the mobile number linked with your Aadhaar."}
            {step === "otp" && `6-digit code sent to +91 ${mobile}.`}
            {step === "details" &&
              "Just your name, email, and a password — then we’ll take you to checkout."}
          </p>
          ...
          {step === "otp" && (
            <div className="mt-6">
              <label className="text-xs text-slate-500">One-time password</label>
              <input
                inputMode="numeric"
                maxLength={6}
                value={otp}
                ...
                placeholder="● ● ● ● ● ●"
                className="mt-2 w-full rounded-xl border border-white/10 bg-slate-900 p-3 text-center text-lg tracking-[0.5em] outline-none focus:border-cyan-400"
              />
